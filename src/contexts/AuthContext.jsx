...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      ...
      console.log(
        "[AuthContext.js] userData:",
        userData ? userData : "missing"
      );
      console.log(
        "[AuthContext.js] accessToken:",
        accessToken ? accessToken.slice(0, 10) + "..." : "missing"
      );
      ...
      if (userData && accessToken) {
        try {
          ...
          if (normalizedUser._id && !socket?.connected) {
            console.log(
              "[AuthContext.js] Initializing socket for user:",
            ...
            socketInstance.on("connect", () => {
              console.log(
                "[AuthContext.js] Socket connected:",
                socketInstance.id
              );
            ...
            socketInstance.on("connect_error", (error) => {
              console.error(
                "[AuthContext.js] Socket connect error:",
                error.message
              );
          ...
          console.error(
            "[AuthContext.js] Error initializing auth:",
            error.message
          );
          localStorage.clear();
          setUser(null);
  ...
  const login = (data) => {
    try {
      ...
      if (normalizedUser._id) {
        console.log(
          "[AuthContext.js] Initializing socket on login for user:",
          normalizedUser.username
        );
        ...
        socketInstance.on("connect", () => {
          console.log(
            "[AuthContext.js] Socket connected on login:",
            socketInstance.id
          );
          setSocketInitialized(true);
        });
        ...
        socketInstance.on("connect_error", (error) => {
          console.error(
            "[AuthContext.js] Socket connect error on login:",
            error.message
          );
          setSocketInitialized(false);
          setSocket(null);
      ...
      message.error(error.message || "Đăng nhập thất bại do lỗi dữ liệu!");
      throw error;
    }
  };
