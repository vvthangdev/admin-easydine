  ...
  return (
    <section className="py-16 px-4 md:px-20 bg-white">
      ...
      <div className="text-center mb-12">
        <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4">Món Ăn Phổ Biến</h2>
        ...
        {categories.map((category) => (
          <button
            ...
            className={`px-6 py-2 rounded-full transition-all duration-300 ${
              activeCategory === category ? 'bg-orange-500 text-white' : 'bg-orange-50 text-gray-700 hover:bg-orange-100'
            }`}
          >
            {category}
          </button>
              ...
              <div className="flex items-center gap-2 mb-4">
                <div className="flex items-center">
                  {[...Array(5)].map((_, index) => (
                    <svg
                      key={index}
                      className={`w-4 h-4 ${index < Math.floor(item.rating || 5) ? 'text-yellow-400' : 'text-gray-300'}`}
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                ...
                <span className="text-sm text-gray-600">({item.reviews || 0} đánh giá)</span>
              </div>

              {/* Category Tag */}
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500">{item.category}</span>
                <button className="text-orange-500 hover:text-orange-600 text-sm font-semibold">Xem chi tiết →</button>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="text-center mt-12">
        <button className="bg-orange-500 text-white px-8 py-3 rounded-full hover:bg-orange-600 transition-colors duration-300 shadow-lg hover:shadow-xl">
          <Link to="/menu">Xem Tất Cả Món Ăn</Link>
        </button>
      </div>
    </section>
  );
