...
export const connectSocket = (token) => {
  console.log(
    "[socket.js] connectSocket called with token:",
    token ? token.slice(0, 10) + "..." : "missing"
  );

  if (socket && socket.connected) {
    console.log(
      "[socket.js] Socket already connected, reusing Socket ID:",
      socket.id
    );
    return socket;
  }
  ...
  socket.on("connect", () => {
    console.log(
      "[socket.js] Connected to Socket.IO server, Socket ID:",
      socket.id
    );
  });
  ...
  socket.on("connect_error", (error) => {
    console.error(
      "[socket.js] Socket connect_error:",
      error.message,
      "Socket ID:",
      socket?.id || "none"
    );
  ...
  socket.on("disconnect", () => {
    console.log(
      "[socket.js] Disconnected from Socket.IO server, Socket ID:",
      socket?.id || "none"
    );
  });
  ...
  socket.on("reconnect", (attempt) => {
    console.log(
      "[socket.js] Reconnected to Socket.IO server after",
      attempt,
      "attempts, Socket ID:",
      socket.id
    );
  ...
  console.log(
    "[socket.js] Socket instance created, Socket ID:",
    socket.id || "pending"
  );
  return socket;
};
...
export const getSocket = () => {
  ...
  if (!socket) {
    ...
    console.log(
      "[socket.js] Returning socket, Socket ID:",
      socket.id,
      "Connected:",
      socket.connected
    );
