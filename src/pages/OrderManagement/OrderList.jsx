...
const OrderList = ({ selectedCustomer, onClearFilter }) => {
    ...
    try {
      ...
      if (!currentOrder) {
        try {
          ...
          message.error(error.message || "Không thể lấy thông tin đơn hàng");
        }
        return;
      }
  ...
  const handleViewCustomerDetails = async (customerId) => {
    try {
      ...
      message.error(
        error.message || "Không thể lấy thông tin chi tiết khách hàng"
      );
    }
  };
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          message.error(error.message || "Xóa đơn hàng không thành công");
        }
      },
    });
  };
