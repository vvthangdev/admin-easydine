...
import { message, Input, Select, Card, Image, Row, Col, Typography, Spin } from 'antd';
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen flex flex-col">
      ...
      {tableId ? (
            ...
            <Row gutter={[16, 16]}>
              {filteredItems.map((item) => (
                <Col xs={24} sm={12} md={8} lg={6} key={item._id}>
                  <Card
                    hoverable
                    cover={<Image src={item.image} alt={item.name} preview={false} style={{ height: 200, objectFit: 'cover' }} />}
                  >
                    <Card.Meta
                      title={item.name}
                      description={
                        <>
                          <Text strong>Giá: {item.price.toLocaleString()} VNĐ</Text>
                          <br />
                          ...
                          <Text>Danh mục: {item.categories.map((cat) => cat.name).join(', ')}</Text>
                          <br />
        ...
        <Text type="danger">Không tìm thấy thông tin bàn. Vui lòng truy cập lại.</Text>
      )}
    </div>
  );
};
